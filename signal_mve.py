...
rolling_sharpe = pl.concat([prior_data, port_returns])
...
).sort('date').shift(-1).alias('bab_sharpe')
).with_columns(
...
for date in dates: # goes through all the dates
...
weight_data = weight_data.with_columns(
    pl.col('return').sort(['barrid', 'date']).shift(-1).over('barrid')
    .alias('lag_return')
).with_columns(
...
with open("mixed_sharpes.txt", "w") as f:
